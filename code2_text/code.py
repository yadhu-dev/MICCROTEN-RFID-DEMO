...
def read_from_serial():
    ...
    while read_loop_active and ser and ser.is_open:
        try:
            if ser.in_waiting > 0:
                ...
                while True:
                    ...
                    packet = serial_buffer[start + 1:end].strip()
                    serial_buffer = serial_buffer[end + 1:]

                    if packet:
                        last_read_data = packet
                        logging.info(f"RFID PACKET OK: {packet}")
                        ...
                        if keyStroke_enabled:
                            pyautogui.typewrite(now.date())
                            pyautogui.press("right")
                            pyautogui.typewrite(now.strftime("%A"))
                            pyautogui.press("right") 
                            pyautogui.typewrite(now.strftime("%H:%M:%S"))
                            pyautogui.press("right") 
